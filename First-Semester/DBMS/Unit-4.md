...
A **transaction** in a DBMS is a sequence of one or more SQL operations that are executed as a **single unit of work**. A transaction ensures that either all the operations are successfully executed (**commit**) or none of them take effect (**rollback**), maintaining database integrity.
...
**1. COMMIT** → Saves all changes made by a transaction permanently in the database.

```sql
COMMIT;
```

**2. ROLLBACK** → Undoes all changes made in the current transaction if an error occurs.

```sql
ROLLBACK;
```

**3. SAVEPOINT** → Creates a point within a transaction to which we can roll back.
